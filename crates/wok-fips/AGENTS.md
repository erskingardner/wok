...
Native FIPS datagram transport for Wok. The platform implementation is Linux
and FreeBSD only and consumes `fips::native::client`; it must never use the
FIPS IPv6/TUN shim or reimplement the native API setup protocol.
